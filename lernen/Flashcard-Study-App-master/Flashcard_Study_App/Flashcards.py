...
def finalize(flashcard_list):
    ...
    the_canvas = tk.Canvas(window,width=550,height=370, highlightthickness=0)
    ...
    flip_button= tk.Button(text="Flip Card",font=("fixedsys",30),command=lambda:flip(flashcard_list,the_canvas,card_index),bg="#2a19e3",fg="white")
    flip_button.pack(side= tk.BOTTOM,pady=40)
    next_button= tk.Button(text="Next",font=("fixedsys",30),command=lambda:next_card(flashcard_list,the_canvas,card_index),bg="#2a19e3",fg="white")
    next_button.place(x=640,y=491)
    prev_button= tk.Button(text="Prev",font=("fixedsys",30),command=lambda:prev_card(flashcard_list,the_canvas,card_index),bg="#2a19e3",fg="white")
...
def start_game():
    ...
    start_label.destroy()
    flashcard_entry= tk.Entry(window,width=80,borderwidth=15)
    flashcard_entry.place(x=245,y=360)
    entry_label= tk.Label(text="Please enter the path of the csv file that contains the flashcard pack you wish to use",font=("fixedsys",30),wraplength=590,bg="#6e17bf",fg="White",justify="center")
    entry_label.pack(pady=60)

    entry_button= tk.Button(text="use flashcard set",font=("fixedsys",30),command=lambda:get_flashcards(flashcard_entry,entry_label,entry_button),bg="#2a19e3",fg="white",padx=73)
...
the_canvas =  tk.Canvas(window,width=1000,height=600, highlightthickness=0)
...
start_label= tk.Label(text="FLASHCARDS",font=("fixedsys",155),bg="#6e17bf",fg="white",pady=90)
start_label.place(x=245,y=100)
start_button= tk.Button(text="START STUDYING",font=("fixedsys",30),command=start_game,bg="#2a19e3",fg="white",anchor=CENTER,padx=99)
